...
BASE_KS="/leonardo_scratch/fast/EUHPC_D36_033/ogas_states_runs/ks800_classical_al"
...
submit_arm() {
    ...
    for seed in "${seeds_array[@]}"; do
        ...
        export LEONARDO_JOB_NAME="ks_${variant_name}_${seed}"
...
echo "=============================================================="
echo "=== LAUNCHING CLASSICAL ACTIVE LEARNING ON KS 1D (10 SEEDS) ==="
echo "=============================================================="

echo "=== 1. M=1 SINGLE MODEL (Forward Loss Scoring) ==="

# 1.1 M=1 Classic AL Top-K
submit_arm "$BASE_KS" "ens1_classic_al_topk" "${ALL_SEEDS[*]}" \
...
submit_arm "$BASE_KS" "ens1_classic_al_sbal" "${ALL_SEEDS[*]}" \
    --set "surrogate.ensemble_size=1" \
    --set "pool.uniform_fraction=0.5" \
...
submit_arm "$BASE_KS" "ens3_classic_al_topk" "${ALL_SEEDS[*]}" \
...
submit_arm "$BASE_KS" "ens3_classic_al_sbal" "${ALL_SEEDS[*]}" \
    --set "surrogate.ensemble_size=3" \
    --set "pool.uniform_fraction=0.5" \
    --set "pool.strategy=classic_al_sbal" \
    --set "pool.classic_al_oversample=10" \
...
submit_arm "$BASE_KS" "ens5_classic_al_topk" "${ALL_SEEDS[*]}" \
...
submit_arm "$BASE_KS" "ens5_classic_al_sbal" "${ALL_SEEDS[*]}" \
    --set "surrogate.ensemble_size=5" \
    --set "pool.uniform_fraction=0.5" \
    --set "pool.strategy=classic_al_sbal" \
    --set "pool.classic_al_oversample=10" \
...
echo "=============================================================="
echo "=== ALL CLASSICAL AL KS EXPERIMENTS SUBMITTED SUCCESSFULLY ==="
echo "=============================================================="
